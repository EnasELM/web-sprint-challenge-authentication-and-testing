...
describe('server.js', () => {
  describe('[POST] /api/auth/login', () => {
    ...
    it('[2] responds with the correct status and message on missing usernamee or password', async () => {
      let res = await request(server).post('/api/auth/login').send({ username: 'yasee' })
      expect(res.body.message).toMatch(/username and password required/i)
      expect(res.status).toBe(422)
      res = await request(server).post('/api/auth/login').send({ password: '12345' })
      expect(res.body.message).toMatch(/username and password required/i)
      expect(res.status).toBe(422)
    }, 750)
  ...
  describe('[POST] /api/auth/register', () => {
    it('[3] creates a new user in the database when client provide username and password', async () => {
    ...
    it('[4] creates a new user in the database when client does not provide  password', async () => {
     let res = await request(server).post('/api/auth/register').send({ username: 'sara' })
     expect(res.body.message).toMatch(/username and password required/i)
    }, 750)
    it('[5] creates a new user in the database when client does not provide username', async () => {
      let res = await request(server).post('/api/auth/register').send({ password: '1234' })
      expect(res.body.message).toMatch(/username and password required/i)
     }, 750)
     it('[6] saves the user with a bcrypted', async () => {
 ...
 describe('[GET] /api/jokes', () => {
    it('[7] requests without a token ', async () => {
     const res = await request(server).get('/api/jokes')
     expect(res.body.message).toMatch(/token required/i)
    }, 750)
    it('[8] requests with an invalid token ', async () => {
